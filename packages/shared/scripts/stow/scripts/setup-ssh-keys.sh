...
setup_ssh_config_include() {
    ...
    if [[ ! -f "$dotfiles_config" ]]; then
        ...
        warning "Make sure you've applied the ssh package: cd ~/dotfiles && ./setup.sh --package ssh"
        info "Will still add Include directive so you can apply the package later"
    else
