...
This package manages the dotfiles-owned SSH include and the hook that wires it into your local SSH config.
...
- `post_link` ensures `Include ~/.ssh/dotfiles.conf` exists in `~/.ssh/config`
- `is_stowed.sh` verifies both the symlinked include file and the include line itself

## Quick Setup (Automated)

Use the home script for interactive key setup after this package has been applied:

```bash
~/scripts/setup-ssh-keys.sh
```

This script is provided by the shared `scripts` package, so the normal root setup flow should apply that package too.
...
The dotfiles SSH configuration is kept separate from your local SSH config. This package's `post_link` hook automatically adds an `Include` directive to `~/.ssh/config`.

**How it works:**
- Dotfiles config source is in `~/dotfiles/packages/shared/ssh/stow/.ssh/dotfiles.conf`
- When applied, it creates `~/.ssh/dotfiles.conf` (symlink to dotfiles)
...
Apply the package to make the dotfiles config available:

```bash
cd ~/dotfiles
./setup.sh --package ssh
```

Then run the interactive key setup script:
