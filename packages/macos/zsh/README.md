...
This package manages the macOS Zsh config.
...
The rest of this file describes the shell-module layout inside `stow/.config/zsh/`.
...
1. `stow/.zshrc` is linked to `~/.zshrc`
2. `~/.zshrc` sources `~/.config/zsh/zshrc`
3. `zshrc` recursively loads all `*.zsh` files from `modules/` directories
